...
func discoverLinks(response *http.Response, baseURL string) []string {
	if response != nil {
		doc, _ := goquery.NewDocumentFromResponse(response) // goquery takes an http response and parses it to make readable by go
		foundUrls := []string{}
		if doc != nil {
...
var tokens = make(chan struct{}, 5) // to use as semaphor so that only 5 threads work at the same time at the crawling process
//learn more about semaphor : https://levelup.gitconnected.com/go-concurrency-pattern-semaphore-9587d45f058d

func crawl(targetURL string, baseURL string) []string {
	fmt.Println(targetURL)
	tokens <- struct{}{} //acquire semaphor
	resp, _ := getRequest(targetURL)
	<-tokens // release semaphor
...
func main() {
	...
	var n int
	n++
	baseDomain := "https://www.tagesschau.de"
	go func() { worklist <- []string{"https://www.tagesschau.de"} }()
